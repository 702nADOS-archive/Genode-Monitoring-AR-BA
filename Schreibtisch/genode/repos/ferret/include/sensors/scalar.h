...
typedef struct
{
    ...
    int64_t   low;       // lower bound for valid range
    int64_t   high;      // upper bound for valid range
    int64_t  val_min;   // minimum value put in so far
    int64_t   val_max;   // maximum value put in so far
    long long       val_count; // number of values inserted
    int64_t   val_sum;   // sum of all values inserted
    int64_t   data;      // finally the data itself
}  ferret_scalar_t;
